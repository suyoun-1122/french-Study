...
const verbForms=w=>Object.values(w?.conjugation||{}).map(stripPronoun).filter(v=>v&&v!=="—");
const verbEligible=w=>{
  if(!w||w.type!=="verb"||w.formPractice===false) return false;
  return new Set(verbForms(w).map(v=>String(v||"").trim()).filter(Boolean)).size>=4;
};
...
(${source.meaning})`,correct:f.fp}
    ...
    const source=verbEligible(q)?q:([...(nearby.filter(verbEligible)),...WORDS.filter(verbEligible)].find(w=>w.id!==q.id)||q);
    ...
    const availableKeys=Object.keys(pronouns).filter(key=>{
      const value=stripPronoun(source.conjugation?.[key]);
      return value&&value!=="—";
    });
    const key=availableKeys.length?availableKeys[(source.id+quiz.index)%availableKeys.length]:"je";
    const pronoun=pronouns[key];
    const correct=stripPronoun(source.conjugation?.[key]||source.word);
    const ownForms=[...new Set(verbForms(source).map(v=>String(v||"").trim()).filter(Boolean))];
    return {skill,label:`현재형에서 주어 ${pronoun}에 맞는 형태를 고르세요`,display:`${source.word} → ${pronoun} ...`,options:uniqueOptions(ownForms,correct),answer:correct,difficulty:level};
  }
...
window.startQuiz=skill=>{
  ...
  if(skill==="form") pool=pool.filter(w=>w.type==="noun"||adjectiveEligible(w)||verbEligible(w));
